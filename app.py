...
def video_tab():
    ...
    def display_metadata(metadata: VideoAnalysis):
        ...
        st.write(
            f"**Tags:** {', '.join(metadata.get('tags', [])) if metadata.get('tags') else 'N/A'}"
        )

    st.header("📹 Video Metadata and Summary Generation")
    st.write(
        "Upload a video to analyze its content and automatically generate metadata and summary."
    )
...
"""
        ...
        converted_boxes = convert_normalized_to_pixel(
...
"""
    ...
    input_method = st.sidebar.radio(
        "Select Image Input Method", ("Upload Image", "Use Camera")
    )
    ...
    if input_method == "Upload Image":
        ...
        if uploaded_file is not None:
            try:
                ...
                st.image(
                    uploaded_image, caption="🖼️ Uploaded Image", use_column_width=True
                )
            except Exception as e:
        ...
        if captured_image is not None:
            try:
                ...
                st.image(
                    uploaded_image, caption="🖼️ Captured Image", use_column_width=True
                )
            except Exception as e:
                st.error(f"❌ Error capturing image: {e}")

    object_name = st.sidebar.text_input(
        "📝 Enter the object to detect", placeholder="e.g., cat, bottle"
    )
        ...
        if uploaded_image is not None:
            ...
            if converted_boxes:
                ...
                annotated_image = draw_bounding_boxes(
                    uploaded_image.copy(), converted_boxes, output_path=None
                )

                # Display the annotated image
                st.image(
                    annotated_image, caption="🖼️ Annotated Image", use_column_width=True
                )
                ...
                st.warning(
                    f"⚠️ No instances of '{object_name}' were found in the image."
                )
        else:
            st.error(
                "⚠️ Please provide an image either by uploading or using the camera."
            )
            st.stop()
...
def audio_tab():
    ...
    uploaded_audio = st.file_uploader(
        "Upload an audio file", type=["mp3", "wav", "aiff", "acc", "ogg", "flac"]
    )
    ...
    if st.button("List Files"):
        ...
        try:
            ...
            if len(file_list) > 0:
                ...
                st.markdown(
                    "<span style='color:red;'>No files found.</span>",
                    unsafe_allow_html=True,
                )  # Display error if no files
        except Exception as e:
